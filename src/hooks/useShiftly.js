...
export function useShiftly() {
  ...
  const saveEntries = (next) => { setEntries(next); localStorage.setItem('shiftly-entries', JSON.stringify(next)); };
  ...
  const stats = useMemo(() => {
    ...
    const weekItems = Object.entries(entries).filter(([date]) => { const value = new Date(`${date}T12:00:00`); return value >= start && value <= end; });
    ...
    const best = monthItems.map(([date, item]) => ({ date, value: calculateShift(item, rates) })).sort((a, b) => b.value - a.value)[0];
